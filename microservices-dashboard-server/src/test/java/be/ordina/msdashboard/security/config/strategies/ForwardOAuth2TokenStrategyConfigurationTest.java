...
public class ForwardOAuth2TokenStrategyConfigurationTest {
	...
	public void propertiesStyledProperty() throws Exception {
		...
		final AnnotationConfigApplicationContext context = load(Config.class, "msdashboard.security.strategies.forward-oauth2-token:mappings,pacts");
	...
	public void yamlStyledProperty() throws Exception {
		...
		final AnnotationConfigApplicationContext context = load(Config.class, "msdashboard.security.strategies.forward-oauth2-token[0]:mappings");
		assertThat(context.containsBean("myBean")).isTrue();
		context.close();
	}
